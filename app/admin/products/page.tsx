  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link href="/admin/products/new" className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-lg hover:opacity-90 transition-opacity shadow-sm">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-muted/50 border-b border-border">
                ...
                <th className="p-4 font-semibold text-sm text-right">Barcode</th>
              </tr>
            </thead>
            <tbody>
              {products.map((product) => (
                <tr key={product.id} className="border-b border-border hover:bg-muted/10 transition-colors">
                  ...
                  <td className="p-4 text-sm text-muted-foreground font-mono text-right">{product.barcode}</td>
                </tr>
              ))}
              {products.length === 0 && (
                <tr>
