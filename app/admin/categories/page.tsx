    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Manage the list of allowed categories for product uploads</p>
        </div>
      </div>
        ...
        <div className="divide-y divide-border">
          {categories.length === 0 ? (
            ...
            categories.map(category => (
              <div key={category.id} className="p-4 px-6 flex items-center justify-between hover:bg-muted/10 transition-colors">
                <div className="flex items-center gap-4">
                  <div className="text-muted-foreground opacity-50"><GripVertical className="w-5 h-5" /></div>
                  <div>
                ...
                <div className="flex items-center gap-2">
                  <form action={deleteCategory.bind(null, category.id)}>
                    <Button 
                      ...
                      title={category._count.products > 0 ? "Cannot delete category in use" : "Delete category"}
                      className={category._count.products > 0 ? "text-muted-foreground" : "text-destructive hover:bg-destructive/10"}
                    >
      ...
      <div className="bg-orange-500/10 border border-orange-500/20 p-4 rounded-lg flex gap-3 text-orange-700 dark:text-orange-400">
        ...
        <div className="text-sm">
          ...
          <p>
            You cannot delete a category that is currently assigned to active products in inventory. 
            If you need to delete a category, first reassign or delete the products using it.
          </p>
        </div>
      </div>
    </div>
